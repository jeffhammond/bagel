...
DFBlock::DFBlock(const DFBlock& o)
 : btas::Tensor3<double>(max(o.adist_shell_->size(mpi__->rank()), max(o.adist_->size(mpi__->rank()), o.asize_)), o.b1size_, o.b2size_),
...
shared_ptr<DFBlock> DFBlock::transform_third(std::shared_ptr<const Matrix> cmat, const bool trans) const {
  ...
  if (!trans)
    dgemm_("N", "N", asize_*b1size_, nocc, b2size_, 1.0, data(), asize_*b1size_, cmat->data(), b2size_, 0.0, out->data(), asize_*b1size_);
  else  // trans -> back transform
    dgemm_("N", "T", asize_*b1size_, nocc, b2size_, 1.0, data(), asize_*b1size_, cmat->data(), nocc, 0.0, out->data(), asize_*b1size_);
...
//
shared_ptr<DFBlock> DFBlock::apply_uhf_2RDM(std::shared_ptr<const btas::Tensor2<double>> amat, std::shared_ptr<const btas::Tensor2<double>> bmat) const {
  assert(b1size_ == b2size_);
  const int nocc = b1size_;
  shared_ptr<DFBlock> out = clone();
  {
...
shared_ptr<DFBlock> DFBlock::apply_2RDM(std::shared_ptr<const btas::Tensor4<double>> rdm, std::shared_ptr<const btas::Tensor2<double>> rdm1, const int nclosed, const int nact) const {
...
shared_ptr<DFBlock> DFBlock::apply_2RDM(std::shared_ptr<const btas::Tensor4<double>> rdm) const {
  shared_ptr<DFBlock> out = clone();
