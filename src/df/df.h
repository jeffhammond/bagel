...
class DFFullDist : public ParallelDF {
  ...
  public:
    ...
    std::shared_ptr<DFFullDist> apply_uhf_2RDM(std::shared_ptr<const btas::Tensor<double>> rdma, std::shared_ptr<const btas::Tensor<double>> rdmb) const;
    // general case with closed orbitals
    std::shared_ptr<DFFullDist> apply_2rdm(std::shared_ptr<const btas::Tensor<double>> rdm, std::shared_ptr<const btas::Tensor<double>> rdm1, const int nclosed, const int nact) const;
    // general case without closed orbitals
    std::shared_ptr<DFFullDist> apply_2rdm(std::shared_ptr<const btas::Tensor<double>> rdm) const;
